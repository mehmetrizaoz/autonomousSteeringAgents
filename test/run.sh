...
echo "-------------" >> static_analysis.out
cppcheck --enable=style test_suites.cpp >> static_analysis.out 2>> static_analysis.out
echo "-------------" >> static_analysis.out

echo "### unit tests result" > unit_test.out 2>> unit_test.out
./test_suites >> unit_test.out 2>> unit_test.out
